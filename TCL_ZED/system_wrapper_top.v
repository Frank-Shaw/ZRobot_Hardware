//Copyright 1986-2014 Xilinx, Inc. All Rights Reserved.
//--------------------------------------------------------------------------------
//Tool Version: Vivado v.2014.2 (win64) Build 928826 Thu Jun  5 18:17:50 MDT 2014
//Date        : Sat Sep 13 16:55:37 2014
//Host        : xshxup12 running 64-bit Service Pack 1  (build 7601)
//Command     : generate_target system_wrapper.bd
//Design      : system_wrapper
//Purpose     : IP block netlist
//--------------------------------------------------------------------------------
`timescale 1 ps / 1 ps

module system_wrapper_top
   (DDR_addr,
    DDR_ba,
    DDR_cas_n,
    DDR_ck_n,
    DDR_ck_p,
    DDR_cke,
    DDR_cs_n,
    DDR_dm,
    DDR_dq,
    DDR_dqs_n,
    DDR_dqs_p,
    DDR_odt,
    DDR_ras_n,
    DDR_reset_n,
    DDR_we_n,
    FIXED_IO_ddr_vrn,
    FIXED_IO_ddr_vrp,
    FIXED_IO_mio,
    FIXED_IO_ps_clk,
    FIXED_IO_ps_porb,
    FIXED_IO_ps_srstb,
    echo,
    leds_8bits_tri_o,
    motorL_dir,
    motorL_phA,
    motorL_phB,
    motorL_pwm,
    motorR_dir,
    motorR_phA,
    motorR_phB,
    motorR_pwm,
    optical_tri_i,
    sws_8bits_tri_i,
    trig);
  inout [14:0]DDR_addr;
  inout [2:0]DDR_ba;
  inout DDR_cas_n;
  inout DDR_ck_n;
  inout DDR_ck_p;
  inout DDR_cke;
  inout DDR_cs_n;
  inout [3:0]DDR_dm;
  inout [31:0]DDR_dq;
  inout [3:0]DDR_dqs_n;
  inout [3:0]DDR_dqs_p;
  inout DDR_odt;
  inout DDR_ras_n;
  inout DDR_reset_n;
  inout DDR_we_n;
  inout FIXED_IO_ddr_vrn;
  inout FIXED_IO_ddr_vrp;
  inout [53:0]FIXED_IO_mio;
  inout FIXED_IO_ps_clk;
  inout FIXED_IO_ps_porb;
  inout FIXED_IO_ps_srstb;
  input [2:0]echo;
  output [7:0]leds_8bits_tri_o;
  output motorL_dir;
  input motorL_phA;
  input motorL_phB;
  output motorL_pwm;
  output motorR_dir;
  input motorR_phA;
  input motorR_phB;
  output motorR_pwm;
  input [4:0]optical_tri_i;
  input [7:0]sws_8bits_tri_i;
  output trig;

  wire [14:0]DDR_addr;
  wire [2:0]DDR_ba;
  wire DDR_cas_n;
  wire DDR_ck_n;
  wire DDR_ck_p;
  wire DDR_cke;
  wire DDR_cs_n;
  wire [3:0]DDR_dm;
  wire [31:0]DDR_dq;
  wire [3:0]DDR_dqs_n;
  wire [3:0]DDR_dqs_p;
  wire DDR_odt;
  wire DDR_ras_n;
  wire DDR_reset_n;
  wire DDR_we_n;
  wire FIXED_IO_ddr_vrn;
  wire FIXED_IO_ddr_vrp;
  wire [53:0]FIXED_IO_mio;
  wire FIXED_IO_ps_clk;
  wire FIXED_IO_ps_porb;
  wire FIXED_IO_ps_srstb;
  wire [2:0]echo;
  wire [7:0]leds_8bits_tri_o;
  wire motorL_dir;
  wire motorL_phA;
  wire motorL_phB;
  wire motorL_pwm;
  wire motorR_dir;
  wire motorR_phA;
  wire motorR_phB;
  wire motorR_pwm;
  wire [4:0]optical_tri_i;
  wire [7:0]sws_8bits_tri_i;
  wire trig;

wire trig_n;
assign trig = ~trig_n;

system_wrapper system_wrapper_i
       (.DDR_addr(DDR_addr),
        .DDR_ba(DDR_ba),
        .DDR_cas_n(DDR_cas_n),
        .DDR_ck_n(DDR_ck_n),
        .DDR_ck_p(DDR_ck_p),
        .DDR_cke(DDR_cke),
        .DDR_cs_n(DDR_cs_n),
        .DDR_dm(DDR_dm),
        .DDR_dq(DDR_dq),
        .DDR_dqs_n(DDR_dqs_n),
        .DDR_dqs_p(DDR_dqs_p),
        .DDR_odt(DDR_odt),
        .DDR_ras_n(DDR_ras_n),
        .DDR_reset_n(DDR_reset_n),
        .DDR_we_n(DDR_we_n),
        .FIXED_IO_ddr_vrn(FIXED_IO_ddr_vrn),
        .FIXED_IO_ddr_vrp(FIXED_IO_ddr_vrp),
        .FIXED_IO_mio(FIXED_IO_mio),
        .FIXED_IO_ps_clk(FIXED_IO_ps_clk),
        .FIXED_IO_ps_porb(FIXED_IO_ps_porb),
        .FIXED_IO_ps_srstb(FIXED_IO_ps_srstb),
        .echo(echo),
        .leds_8bits_tri_o(leds_8bits_tri_o),
        .motorL_dir(motorL_dir),
        .motorL_phA(motorL_phA),
        .motorL_phB(motorL_phB),
        .motorL_pwm(motorL_pwm),
        .motorR_dir(motorR_dir),
        .motorR_phA(motorR_phA),
        .motorR_phB(motorR_phB),
        .motorR_pwm(motorR_pwm),
        .optical_tri_i(optical_tri_i),
        .sws_8bits_tri_i(sws_8bits_tri_i),
        .trig(trig_n));
endmodule
